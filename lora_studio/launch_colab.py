...
if IN_COLAB:
    ...
    if _launch_mode == "proxy_url":
        ...
        <script>
        (function() {
            var btn = document.getElementById('lora-studio-popout-btn');
            if (!btn) return;
        ...
        _POPOUT_BTN_HTML = (
            '<button id="lora-studio-popout-btn" style="'
        ...
        try:
            ...
            _iframe_ok = True
            display(_HTML(f"""
            <div style="margin:8px 0 4px;padding:10px 16px;background:#141414;border:2px solid #7b68ee;border-radius:12px;font-family:monospace;display:flex;align-items:center;flex-wrap:wrap;gap:6px;">
        ...
        if not _iframe_ok:
            try:
                from google.colab import output as _colab_output
                _colab_output.serve_kernel_port_as_window(PORT, anchor_text="⚡ Click to open LoRA Studio")
                _launch_mode = "window"
                display(_HTML(f"""
                <div style="margin:8px 0;padding:10px 16px;background:#141414;border:2px solid #7b68ee;border-radius:12px;font-family:monospace;display:flex;align-items:center;flex-wrap:wrap;gap:6px;">
                ...
                try:
                    ...
                    (async () => {
                        ...
                        iframe.width = '100%%';
                        iframe.height = '820';
                        iframe.style.border = '2px solid #7b68ee';
