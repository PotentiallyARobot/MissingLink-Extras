...
class PromptFlowInterpreter:
    ...
    # Single call — returns raw text for local, optionally parsed JSON for openai.
    ...
    # Calls the LLM once per item in a list. Collects raw text strings into a list.
    # The local LLM is never asked for any structure here — just one sentence each.
        ...
        for idx, step in enumerate(steps, 1):
            ...
            try:
                if step_type == "llm_call":
                    # Returns raw text (local) or text/parsed (openai).
                    # Stored under output_key so steps.<id>.<output_key> resolves correctly.
                    ...
                    # Returns {output_key: [sentence, sentence, ...]}
