...
def qwen_instruction_edit(
    ...
    if source is None:
        raise ValueError("Upload a source image for Qwen Edit.")
    prompt = str(prompt or "").strip()
    if not prompt:
        raise ValueError("Enter a Qwen edit instruction.")

    source = resize_image(source, max_side=max_side)
    refs = []
    for image in (reference_images or [])[:4]:
        if image is None:
            continue
        refs.append(resize_image(image, max_side=max_side))
    ...
    pipe = ensure_qwen_camera_engine()
    check_stop()
    _emit_progress("Qwen instruction edit", pct=18, step=0, step_total=steps)
    print(
        f"[QWEN EDIT] refs={len(refs)} · seed={seed} · steps={steps} · {prompt[:240]}",
        flush=True,
    )
    ...
    check_stop()
    outputs = getattr(result, "images", None) or []
    if not outputs:
        raise RuntimeError("Qwen Edit returned no image.")
    image = outputs[0].convert("RGB")
    elapsed = time.time() - started
    _emit_progress("Saving Qwen edit", pct=94, step=steps, step_total=steps)
    path = save_image(
        image,
        "qwen-edit",
        prompt,
    ...
    status = (
        f"Qwen Edit complete · {len(refs)} reference"
        + ("" if len(refs) == 1 else "s")
        + f" · seed {seed} · {elapsed:.1f}s"
    )
    _emit_progress("Qwen edit complete", pct=100, step=steps, step_total=steps)
    return path, seed, status, path
...
def _request_extra_loras(value):
    if isinstance(value, str):
        try:
            value = json.loads(value or "[]")
        except Exception:
            value = []
    return _normalize_extra_loras(value or [])
...
def _submit_job(kind, params, *, label, mode, thumb=None, batch_total=0):
    ...
    if "extra_loras" in params:
        params["extra_loras"] = _normalize_extra_loras(params.get("extra_loras") or [])
    else:
        with _EXTRA_LORA_LOCK:
...
def _execute_job(jid):
    ...
    with JOB_LOCK:
        ...
        job.update(
            ...
            stage="Starting GPU",
            detail=(
                "Preparing Qwen 2511 on the local GPU."
                if job.get("kind") in {"camera", "qedit"}
    ...
    if kind == "qedit":
        source = _load_job_image(params["source_path"])
        references = [
            _load_job_image(path)
            for path in (params.get("reference_paths") or [])[:4]
        ]
        path, seed, status, download_path = qwen_instruction_edit(
            source=source,
        ...
        return {"image": _output_url(path), "seed": seed, "status": status,
                "download": _output_url(download_path), "engine": "qwen_edit"}
    ...
    try:
        ...
        jid = _submit_job(
            "text",
            {
                ...
                "extra_loras": _request_extra_loras(source.get("extra_loras")),
            },
    ...
    try:
        jid = _submit_job(
            "image",
            {
                ...
                "extra_loras": _request_extra_loras(request.form.get("extra_loras")),
            },
...
@app.post("/api/qwen_edit")
def api_qwen_edit():
    source = _pil_upload(request.files.get("source"))
    if source is None:
        return _json_error("Upload a source image for Qwen Edit.")
    source_path = _save_job_image(source, "qwen_edit_source")
    reference_paths = _save_job_images(
        _pil_uploads(request.files.getlist("references")),
        "qwen_edit_reference",
    )
    try:
        jid = _submit_job(
            ...
            label="Qwen Edit", mode="qwen_edit", thumb=_input_url(source_path),
        )
        return jsonify(ok=True, id=jid, queued=True, access=_ml_public_access_state()), 202
    except Exception as exc:
        return _json_error(exc, 409 if "Queue is full" in str(exc) else 400)
...
.tab-lora-stack{margin-top:12px;padding:10px;border:1px solid #2a2b31;border-radius:8px;background:#0b0b0d}.tab-lora-head{display:flex;align-items:center;justify-content:space-between;gap:8px;font:700 9px var(--font-mono);letter-spacing:.8px;color:#c8cad1}.tab-lora-count{color:var(--gold);font-size:8px}.tab-lora-actions{margin-top:7px!important}.tab-lora-list{max-height:250px;overflow:auto;scrollbar-width:none;-ms-overflow-style:none}.tab-lora-list::-webkit-scrollbar{display:none;width:0;height:0}.tab-lora-stack .lora-row{grid-template-columns:24px minmax(0,1fr) 70px 0}.tab-lora-stack .lora-delete{display:none!important}.tab-lora-stack .lora-meta{font-size:7px}.tab-lora-stack .lora-strength{height:30px!important}.tab-lora-stack .status{margin-top:7px}.lora-inline-installer{margin-top:0!important}.shared-lora-card{display:none!important}
</style>
</head>
<body>
<div class="app-shell">
  ...
  <div class="app-body">
    <aside class="studio-sidebar">
      <nav class="mode-tabs">
        ...
        <button class="tab" data-tab="edit">KREA EDIT</button>
        <button class="tab" data-tab="qedit">QWEN EDIT</button>
      ...
      <div class="sidebar-scroll">
        ...
        <section id="panel_text" class="controlpanel active">
          <div class="card">
            ...
            <div class="cardbody">
              ...
              <div class="tab-lora-stack" data-lora-tab="text">
                <div class="tab-lora-head"><span>LORA STACK</span><span class="tab-lora-count" data-lora-count="text">0/4</span></div>
                <div class="sectionhint">This stack applies only to new jobs from this tab. Up to four user LoRAs are snapshotted when you queue the job.</div>
                <div class="actions tab-lora-actions">
                  <button class="lora-add-tab" type="button" data-lora-tab="text">+ ADD LORA</button>
                  <button class="secondary lora-refresh-tab" type="button" data-lora-tab="text">REFRESH</button>
                </div>
                <div id="lora_list_text" class="lora-list tab-lora-list"><div class="empty">Loading LoRAs…</div></div>
                <div class="status lora-tab-status" data-lora-status="text">LoRA stack ready.</div>
              </div>
        ...
        <section id="panel_image" class="controlpanel">
          <div class="card">
            ...
            <div class="cardbody">
              ...
              <div class="tab-lora-stack" data-lora-tab="image">
                <div class="tab-lora-head"><span>LORA STACK</span><span class="tab-lora-count" data-lora-count="image">0/4</span></div>
                <div class="sectionhint">This stack applies only to new jobs from this tab. Up to four user LoRAs are snapshotted when you queue the job.</div>
                <div class="actions tab-lora-actions">
                  <button class="lora-add-tab" type="button" data-lora-tab="image">+ ADD LORA</button>
                  <button class="secondary lora-refresh-tab" type="button" data-lora-tab="image">REFRESH</button>
                </div>
                <div id="lora_list_image" class="lora-list tab-lora-list"><div class="empty">Loading LoRAs…</div></div>
                <div class="status lora-tab-status" data-lora-status="image">LoRA stack ready.</div>
              </div>
        ...
        <section id="panel_edit" class="controlpanel">
          <div class="card">
            ...
            <div class="cardbody">
              ...
              <div class="tab-lora-stack" data-lora-tab="edit">
                <div class="tab-lora-head"><span>LORA STACK</span><span class="tab-lora-count" data-lora-count="edit">0/4</span></div>
                <div class="sectionhint">This stack applies only to new jobs from this tab. Up to four user LoRAs are snapshotted when you queue the job.</div>
                <div class="actions tab-lora-actions">
                  <button class="lora-add-tab" type="button" data-lora-tab="edit">+ ADD LORA</button>
                  <button class="secondary lora-refresh-tab" type="button" data-lora-tab="edit">REFRESH</button>
                </div>
                <div id="lora_list_edit" class="lora-list tab-lora-list"><div class="empty">Loading LoRAs…</div></div>
                <div class="status lora-tab-status" data-lora-status="edit">LoRA stack ready.</div>
              </div>
        ...
        <section id="panel_inpaint" class="controlpanel">
          <div class="card">
            ...
            <div class="cardbody">
              ...
              <div class="tab-lora-stack" data-lora-tab="inpaint">
                <div class="tab-lora-head"><span>LORA STACK</span><span class="tab-lora-count" data-lora-count="inpaint">0/4</span></div>
                <div class="sectionhint">This stack applies only to new jobs from this tab. Up to four user LoRAs are snapshotted when you queue the job.</div>
                <div class="actions tab-lora-actions">
                  <button class="lora-add-tab" type="button" data-lora-tab="inpaint">+ ADD LORA</button>
                  <button class="secondary lora-refresh-tab" type="button" data-lora-tab="inpaint">REFRESH</button>
                </div>
                <div id="lora_list_inpaint" class="lora-list tab-lora-list"><div class="empty">Loading LoRAs…</div></div>
                <div class="status lora-tab-status" data-lora-status="inpaint">LoRA stack ready.</div>
              </div>
        ...
        <section id="panel_qedit" class="controlpanel">
          <div class="card">
            <div class="cardtitle">Qwen Edit · 2511 NF4</div>
            <div class="cardbody">
              <div class="engine-note"><b>Dedicated Qwen edit engine.</b> This uses the same Qwen Image Edit 2511 NF4 runtime as Qwen Camera. Krea2 is fully released before the first Qwen job; Qwen stays warm when moving between Qwen Edit and Qwen Camera.</div>
              <label>Source image</label>
              <input id="qe_source" class="nativefile" type="file" accept="image/*">
              <div class="uploadrow"><button id="qe_source_pick" class="filepick" type="button"><span class="filepickicon">＋</span><span>Choose image</span></button><span id="qe_source_name" class="filemeta">No image selected</span></div>
              <div id="qe_preview" class="preview"><div class="empty">Choose the image you want to edit.</div></div>

              <label>Edit instruction</label>
              <textarea id="qe_prompt" placeholder="Change the jacket to red while preserving the person, pose and scene…"></textarea>

              <label>Additional image references · optional · up to 4</label>
              <input id="qe_refs" class="nativefile" type="file" accept="image/*" multiple>
              <div class="uploadrow"><button id="qe_refs_pick" class="filepick" type="button"><span class="filepickicon">＋</span><span>Add references</span></button><span id="qe_refs_name" class="filemeta">No additional references</span></div>

              <label>Negative prompt · optional</label>
              <textarea id="qe_negative" placeholder="Optional things to avoid"></textarea>
              <div class="split">
                <div><label>Maximum side</label><select id="qe_max"></select></div>
                <div><label>Seed (-1 = random)</label><input id="qe_seed" type="number" value="-1"></div>
              </div>
              <details>
                <summary>Qwen Edit settings</summary>
                <div class="inside">
                  <div class="grid3">
                    <div><label>Steps</label><input id="qe_steps" type="number" min="1" max="80" value="30"></div>
                    <div><label>Guidance</label><input id="qe_guidance" type="number" min="0" max="20" step=".1" value="1.0"></div>
                    <div><label>True CFG</label><input id="qe_true_cfg" type="number" min="0" max="20" step=".1" value="4.0"></div>
                  </div>
                  <div class="minihint">Use the source as Image 1. Optional references are passed directly to Qwen as additional images for subject, product, scene or visual-context editing.</div>
                </div>
              </details>
              <div class="actions"><button id="qe_generate">QWEN EDIT</button></div>
              <div id="qe_status" class="status">Ready. First Qwen job swaps Krea2 out and loads the NF4 Qwen 2511 engine.</div>
              <a id="qe_download" class="hidden" target="_blank"></a>
            </div>
          </div>
        </section>
        ...
        <section id="panel_camera" class="controlpanel">
          <div class="card">
            <div class="cardtitle">Qwen Camera · 2511 + Multi-Angles</div>
            <div class="cardbody">
        ...
        <section id="panel_batch" class="controlpanel">
          ...
          <div class="card">
            ...
            <div class="cardbody">
              ...
              <div class="tab-lora-stack" data-lora-tab="batch">
                <div class="tab-lora-head"><span>LORA STACK</span><span class="tab-lora-count" data-lora-count="batch">0/4</span></div>
                <div class="sectionhint">This stack applies only to new jobs from this tab. Up to four user LoRAs are snapshotted when you queue the job.</div>
                <div class="actions tab-lora-actions">
                  <button class="lora-add-tab" type="button" data-lora-tab="batch">+ ADD LORA</button>
                  <button class="secondary lora-refresh-tab" type="button" data-lora-tab="batch">REFRESH</button>
                </div>
                <div id="lora_list_batch" class="lora-list tab-lora-list"><div class="empty">Loading LoRAs…</div></div>
                <div class="status lora-tab-status" data-lora-status="batch">LoRA stack ready.</div>
              </div>
              <div class="actions">
...
<div id="lora_modal" class="auto-modal hidden" aria-hidden="true">
  <div class="auto-modal-shell">
    <div class="auto-modal-head">
      <span class="auto-modal-title">ADD LORA</span>
      <button id="lora_install_hide" type="button" class="auto-modal-close">×</button>
    </div>
    <div class="auto-modal-body">
      <div id="lora_install_inline" class="lora-inline-installer">
        <label>Source</label>
...
fillSelect('qe_max', dims, __DEFAULT_IMAGE_MAX_SIDE__);
...
// ====================================================================
// USER LoRA MANAGER · PER-KREA-TAB STACKS
// ====================================================================
const KREA_LORA_TABS=['text','image','edit','inpaint','batch'];
let loraLibrary=[];
let loraStacks={text:[],image:[],edit:[],inpaint:[],batch:[]};
let loraStacksInitialized=false;
let loraInstallerTargetTab='text';
...
function _normalizedClientStack(stack){
  const out=[];const seen=new Set();
  (stack||[]).forEach(item=>{
    const name=String(item?.name||''); if(!name||seen.has(name))return;
    const row=loraLibrary.find(x=>x.name===name); if(!row||row.reserved||!row.valid)return;
    let strength=Number(item?.strength??1);if(!Number.isFinite(strength))strength=1;strength=Math.max(-2,Math.min(2,strength));
    out.push({name,strength});seen.add(name);
  });
  return out.slice(0,4);
}
function _initialStackFromLibrary(){
  return _normalizedClientStack(loraLibrary.filter(x=>x.active&&!x.reserved).map(x=>({name:x.name,strength:Number(x.strength??1)})));
}
function activeLoraStackForTab(tab){return _normalizedClientStack(loraStacks[tab]||[])}
function _setTabLoraStatus(tab,text,kind=''){
  const el=document.querySelector(`[data-lora-status="${tab}"]`);
  if(!el)return;
  el.textContent=text||'';
  el.className='status lora-tab-status'+(kind?' '+kind:'');
}
function renderLoraTab(tab,maxActive=4){
  const root=$('lora_list_'+tab);if(!root)return;
  root.innerHTML='';
  const stack=activeLoraStackForTab(tab);
  const selected=new Map(stack.map(x=>[x.name,x.strength]));
  const count=document.querySelector(`[data-lora-count="${tab}"]`);
  if(count)count.textContent=stack.length+'/'+maxActive;
  const rows=loraLibrary.filter(x=>!x.reserved);
  if(!rows.length){
    root.innerHTML='<div class="empty">No user LoRAs installed. Click + ADD LORA.</div>';
    return;
  }
  rows.forEach(item=>{
    const row=document.createElement('div');
    row.className='lora-row'+(!item.valid?' reserved':'');
    const check=document.createElement('input');
    check.type='checkbox';check.style.width='auto';check.checked=selected.has(item.name);check.disabled=!item.valid;
    const info=document.createElement('div');
    const name=document.createElement('div');name.className='lora-name';name.textContent=item.name;
    const meta=document.createElement('div');meta.className='lora-meta';meta.textContent=_fmtLoraBytes(item.bytes)+(item.valid?'':' · INVALID');
    info.append(name,meta);
    const strength=document.createElement('input');
    strength.className='lora-strength';strength.type='number';strength.min='-2';strength.max='2';strength.step='.05';
    strength.value=Number(selected.get(item.name)??item.strength??1).toFixed(2);strength.disabled=!item.valid;
    const spacer=document.createElement('span');
    const sync=()=>{
      let next=activeLoraStackForTab(tab).filter(x=>x.name!==item.name);
      if(check.checked){
        if(next.length>=maxActive){
          check.checked=false;
          _setTabLoraStatus(tab,'Up to '+maxActive+' LoRAs can be active on this tab.','bad');
          return;
        }
        let s=Number(strength.value||1);if(!Number.isFinite(s))s=1;s=Math.max(-2,Math.min(2,s));
        next.push({name:item.name,strength:s});
      }
      loraStacks[tab]=next.slice(0,maxActive);
      renderLoraTab(tab,maxActive);
      _setTabLoraStatus(tab,(loraStacks[tab].length?loraStacks[tab].length+' active LoRA'+(loraStacks[tab].length===1?'':'s'):'No active user LoRAs')+' for new '+tab+' jobs.','good');
    };
    check.onchange=sync;
    strength.onchange=()=>{if(check.checked)sync()};
    row.append(check,info,strength,spacer);
    root.appendChild(row);
  });
}
function renderLoraLibrary(items,maxActive=4){
  loraLibrary=items||[];
  if(!loraStacksInitialized){
    const initial=_initialStackFromLibrary();
    KREA_LORA_TABS.forEach(tab=>{loraStacks[tab]=initial.map(x=>({...x}))});
    loraStacksInitialized=true;
  }else{
    KREA_LORA_TABS.forEach(tab=>{loraStacks[tab]=_normalizedClientStack(loraStacks[tab])});
  }
  KREA_LORA_TABS.forEach(tab=>renderLoraTab(tab,maxActive));
}
async function refreshLoras(){
  try{
    const d=await fetchJson('/api/loras');
    renderLoraLibrary(d.items||[],d.max_active||4);
    KREA_LORA_TABS.forEach(tab=>_setTabLoraStatus(tab,'LoRA library ready · '+activeLoraStackForTab(tab).length+' active for this tab.','good'));
  }catch(e){
    KREA_LORA_TABS.forEach(tab=>_setTabLoraStatus(tab,e.message,'bad'));
  }
}
document.addEventListener('click',e=>{
  const add=e.target.closest('.lora-add-tab');
  if(add){
    e.preventDefault();
    loraInstallerTargetTab=add.dataset.loraTab||activeStudioTab||'text';
    openLoraInstallInline();
    return;
  }
  const refresh=e.target.closest('.lora-refresh-tab');
  if(refresh){
    e.preventDefault();
    refreshLoras();
    return;
...
function openLoraInstallInline(){
  const modal=$('lora_modal');if(!modal)return;
...
$('lora_modal').addEventListener('click',e=>{if(e.target===$('lora_modal'))closeLoraInstallInline()});
  ...
  const tick=async()=>{
    try{
      ...
      if(d.status==='done'){
        ...
        const installed=String(d.file||d.filename||'');
        if(installed){
          const tab=KREA_LORA_TABS.includes(loraInstallerTargetTab)?loraInstallerTargetTab:'text';
          const next=activeLoraStackForTab(tab).filter(x=>x.name!==installed);
          if(next.length<4)next.push({name:installed,strength:1});
          loraStacks[tab]=next.slice(0,4);
        }
        await refreshLoras();
        setStatus('lora_download_text','Installed '+String(d.file||d.filename||'LoRA')+' · added to '+loraInstallerTargetTab+' LoRA stack.','good');
        _setTabLoraStatus(loraInstallerTargetTab,'LoRA installed and selected for new jobs from this tab.','good');
      ...
      if(d.status==='error'){
        ...
        _setTabLoraStatus(loraInstallerTargetTab,String(d.error||'LoRA download failed.'),'bad');
        setLoraDownloadBusy(false);
        loraActiveDownloadId='';
        return;
      }
      ...
      _setTabLoraStatus(loraInstallerTargetTab,String(e.message||e),'bad');
      setLoraDownloadBusy(false);
      loraActiveDownloadId='';
    }
  };
...
async function startLoraDownload(){
  ...
  try{
    ...
    _setTabLoraStatus(loraInstallerTargetTab,'LoRA download started in the background.','good');
    ...
    _setTabLoraStatus(loraInstallerTargetTab,String(err.message||err),'bad');
    setLoraDownloadBusy(false);
  }
}
...
const SINGLE_STAGE_IDS=['t_result','i_result','e_result','in_result','qe_result','c_result'];
const SINGLE_STAGE_DOWNLOADS={t_result:'t_download',i_result:'i_download',e_result:'e_download',in_result:'in_download',qe_result:'qe_download',c_result:'cam_download'};
...
async function assignImageUrlToEditors(url){
  ...
  const dtQ=new DataTransfer(); dtQ.items.add(file); $('qe_source').files=dtQ.files; $('qe_source').dispatchEvent(new Event('change'));
...
const ML_GENERATE_BY_TAB={text:'t_generate',image:'i_generate',edit:'e_generate',inpaint:'in_generate',qedit:'qe_generate',camera:'c_generate',batch:'b_generate'};
...
$('stage_assign').onclick=async()=>{
  try{
    ...
    setStatus('qe_status','Staged image assigned to Qwen Edit.','good');
    setStatus('cam_status','Staged image assigned to Qwen Camera.','good');
  }catch(e){
...
bindFilePicker('qe_source','qe_source_pick','qe_source_name',{previewId:'qe_preview',emptyText:'Choose the image you want to edit.'});
bindFilePicker('c_source','c_source_pick','c_source_name',{previewId:'c_preview',emptyText:'Choose the image you want to re-stage.'});
$('qe_refs_pick').onclick=()=>$('qe_refs').click();
$('qe_refs').addEventListener('change',()=>{
  const n=Math.min(4,($('qe_refs').files||[]).length);
  $('qe_refs_name').textContent=n?(n+' reference image'+(n===1?'':'s')+' selected'):'No additional references';
  $('qe_refs_name').classList.toggle('selected',!!n);
});
...
$('qe_generate').onclick=async()=>{
  const source=$('qe_source').files[0];
  if(!source){setStatus('qe_status','Upload or assign a source image first.','bad');return}
  const fd=new FormData();
  fd.append('source',source);
  fd.append('prompt',$('qe_prompt').value);
  fd.append('negative_prompt',$('qe_negative').value);
  fd.append('max_side',$('qe_max').value);
  fd.append('seed',$('qe_seed').value);
  fd.append('steps',$('qe_steps').value);
  fd.append('guidance_scale',$('qe_guidance').value);
  fd.append('true_cfg_scale',$('qe_true_cfg').value);
  [...($('qe_refs').files||[])].slice(0,4).forEach(file=>fd.append('references',file,file.name));
  try{
    const d=await fetchJson('/api/qwen_edit',{method:'POST',body:fd});
    setStatus('qe_status','Queued · '+d.id+' · Qwen 2511 will hard-swap in when this job reaches the GPU.','');
    pollJobs();
...
$('t_generate').onclick=async()=>{
  try{
    ...
    fd.append('extra_loras',JSON.stringify(activeLoraStackForTab('text')));
...
$('i_generate').onclick=async()=>{
  ...
  appendReferenceFilesToFormData(fd);
  fd.append('extra_loras',JSON.stringify(activeLoraStackForTab('image')));
  try{
...
$('e_generate').onclick=async()=>{
  ...
  appendReferenceFilesToFormData(fd);
  fd.append('extra_loras',JSON.stringify(activeLoraStackForTab('edit')));
  try{
...
$('in_generate').onclick=async()=>{
  ...
  try{
    ...
    appendReferenceFilesToFormData(fd);
    fd.append('extra_loras',JSON.stringify(activeLoraStackForTab('inpaint')));
...
function batchForm(){
  ...
  appendReferenceFilesToFormData(fd);
  fd.append('extra_loras',JSON.stringify(activeLoraStackForTab('batch')));
  return fd;
}
...
function applyFinishedJob(job){
  ...
  }else if(job.kind==='qedit'){
    if(job.status==='done'&&r.image){setSharedSingleImage(r.image)}
    setStatus('qe_status',message,bad?'bad':(job.status==='done'?'good':''));
