...
{_h3_prompt_contract(mode, duration)}'''
    ...
    *{scrollbar-width:none}*::-webkit-scrollbar{display:none;width:0;height:0}
    ...
    <div class=cameracontrol>
      <div class=cameratitle><strong>🎥 Camera Pose</strong><div class=switchrow><input id=camera_pose_enabled type=checkbox checked autocomplete=off><label for=camera_pose_enabled>USE CAMERA CONTROL</label></div></div>
      <div class=cameraviz id=camera_viz aria-label="Camera pose visualization">
        <svg viewBox="0 0 360 190" preserveAspectRatio="none" aria-hidden=true>
          <defs><pattern id=cam_grid width="18" height="18" patternUnits="userSpaceOnUse"><path d="M 18 0 L 0 0 0 18" fill="none" class=camgrid/></pattern></defs>
          <path d="M20 125 L180 78 L340 125 L180 174 Z" fill="url(#cam_grid)" opacity=.95/>
          <ellipse cx="180" cy="126" rx="130" ry="48" class=camorbit/>
          <ellipse cx="180" cy="126" rx="102" ry="37" class=camorbitghost/>
          <polygon points="166,93 196,101 196,141 166,132" class=subjectplane/>
          <line x1="181" y1="94" x2="181" y2="137" class=subjectline/>
          <circle cx="181" cy="116" r="10" class=subjectdot/>
          <circle cx="62" cy="154" r="8" class=axisdot/>
          <line id=camera_ray x1="180" y1="126" x2="181" y2="116" class=camray/>
          <line id=camera_stem x1="180" y1="126" x2="180" y2="85" class=camstem/>
          <circle id=camera_dot cx="180" cy="126" r="11" class=camdot/>
          <text id=camera_dot_label x="180" y="129" text-anchor="middle" class=camlabel>CAM</text>
        </svg>
        <div id=camera_readout class=camreadout>Front (0°) · Elevated (20°)</div>
      </div>
      <div class=cameraform>
        <div><label>Azimuth</label><select id=camera_azimuth><option value=0 selected>Front (0°)</option><option value=45>Front-right (45°)</option><option value=90>Right (90°)</option><option value=135>Back-right (135°)</option><option value=180>Back (180°)</option><option value=-135>Back-left (-135°)</option><option value=-90>Left (-90°)</option><option value=-45>Front-left (-45°)</option></select></div>
        <div><label>Elevation</label><select id=camera_elevation><option value=-20>Low (-20°)</option><option value=0>Eye level (0°)</option><option value=20 selected>Elevated (20°)</option><option value=45>High (45°)</option><option value=75>Overhead (75°)</option></select></div>
      </div>
      <div class=camerahint>Camera Pose is translated into natural-language camera placement for H3. It leaves the visible prompt untouched and is applied only to the submitted request.</div>
    </div>
    ...
    $('auto_prompt_btn').onclick=async()=>{
      ...
      const fd=new FormData();
      fd.append('rough_prompt',promptWithCameraPose());
    ...
    function cameraPoseState(){
      const enabled=!!($('camera_pose_enabled')&&$('camera_pose_enabled').checked);
      const az=Number($('camera_azimuth')?.value||0),el=Number($('camera_elevation')?.value||0);
      const azNames={"0":"front","45":"front-right three-quarter","90":"right profile","135":"back-right three-quarter","180":"back","-135":"back-left three-quarter","-90":"left profile","-45":"front-left three-quarter"};
      const elNames={"-20":"low-angle","0":"eye-level","20":"slightly elevated","45":"high-angle","75":"near-overhead"};
      return {enabled,azimuth:az,elevation:el,azimuthLabel:azNames[String(az)]||`${az}° azimuth`,elevationLabel:elNames[String(el)]||`${el}° elevation`};
    }
    function cameraPoseDirective(){
      const c=cameraPoseState();
      if(!c.enabled)return '';
      return `Camera placement: ${c.azimuthLabel} view at ${c.azimuth}° azimuth, ${c.elevationLabel} at ${c.elevation}° elevation. Keep this camera side and elevation as the primary framing unless the prompt explicitly requests camera movement.`;
    }
    function promptWithCameraPose(){
      const raw=$('prompt')?.value||'',cam=cameraPoseDirective();
      return cam?`${raw}${raw.trim()?'\n\n':''}${cam}`:raw;
    }
    function updateCameraPoseViz(){
      if(!$('camera_dot'))return;
      const c=cameraPoseState(),rad=c.azimuth*Math.PI/180,cx=180,cy=126,rx=130,ry=48;
      const x=cx+rx*Math.sin(rad),baseY=cy+ry*Math.cos(rad),lift=Math.max(-16,Math.min(35,c.elevation*.46)),y=baseY-lift;
      $('camera_dot').setAttribute('cx',x.toFixed(1));$('camera_dot').setAttribute('cy',y.toFixed(1));
      $('camera_dot_label').setAttribute('x',x.toFixed(1));$('camera_dot_label').setAttribute('y',(y+2.6).toFixed(1));
      $('camera_ray').setAttribute('x1',x.toFixed(1));$('camera_ray').setAttribute('y1',y.toFixed(1));
      $('camera_stem').setAttribute('x1',x.toFixed(1));$('camera_stem').setAttribute('y1',y.toFixed(1));$('camera_stem').setAttribute('x2',x.toFixed(1));$('camera_stem').setAttribute('y2',(y-24-Math.max(0,c.elevation*.25)).toFixed(1));
      $('camera_readout').textContent=`${$('camera_azimuth').selectedOptions[0]?.textContent||c.azimuth+'°'} · ${$('camera_elevation').selectedOptions[0]?.textContent||c.elevation+'°'}`;
      $('camera_viz').style.opacity=c.enabled?'1':'.42';$('camera_azimuth').disabled=!c.enabled;$('camera_elevation').disabled=!c.enabled;
    }
    if($('camera_pose_enabled'))$('camera_pose_enabled').addEventListener('change',updateCameraPoseViz);
    if($('camera_azimuth'))$('camera_azimuth').addEventListener('change',updateCameraPoseViz);
    if($('camera_elevation'))$('camera_elevation').addEventListener('change',updateCameraPoseViz);
    updateCameraPoseViz();
    ...
    async function submitGeneration(){
      ...
      fd.append('prompt',promptWithCameraPose());
      for(const k of ['width','height','duration','frames','length_mode','playback_speed','image_fit','steps','seed','denoise','shift_video','shift_audio','sparse_percent','sampler_name','scheduler','weight_dtype','unet','use_stage_last'])fd.append(k,$(k).value);
