...
def worldeditor_page():
    html_path = pathlib.Path(__file__).parent / "worldeditor.html"
    if html_path.exists():
        return send_file(str(html_path), mimetype="text/html")
    else:
        return Response("<h1>worldeditor.html not found</h1>", mimetype="text/html", status=404)


@app.route("/api/worldeditor/models")
def api_worldeditor_models():
    """List all GLB files from completed jobs for use in the world editor."""
    models = []
    seen_paths = set()

    # Scan all completed job results for GLB files
    for jid, job in list(jobs.items()):
        if job.get("status") != "done":
            continue
        for result in job.get("results", []):
            glb_path = result.get("glb", "")
            if glb_path and glb_path not in seen_paths and os.path.isfile(glb_path):
                seen_paths.add(glb_path)
                name = result.get("name", os.path.basename(glb_path).replace(".glb", ""))
                models.append({
                    "name": name,
                    "url": f"/api/file?p={glb_path}",
                    "path": glb_path,
                })

    # Also scan the default output directory for any GLBs
    default_out = "/content/drive/MyDrive/trellis_models_out"
    if os.path.isdir(default_out):
        for root_dir, dirs, filenames in os.walk(default_out):
            for fn in filenames:
...
def api_file():
    ...
    # Always allow the default trellis output directory (for world editor)
    _default_out = "/content/drive/MyDrive/trellis_models_out"
    if os.path.isdir(_default_out):
        allowed_dirs.add(os.path.realpath(_default_out))
