...
# H3 memory optimization, isolated Sage/SDPA probe, exact conditioning cache.
# Default FAST recipe: 4-step Turbo. ULTRA and TaoMate are quality/speed choices.
# Optional H3_L40S_COMPILE=1 uses Comfy's experimental compiler wrapper.
# This edition has passed static/mocked checks, NOT an L40S render benchmark.
# See h3_studio_l40s_README.md for runtime setup, controls and validation.
# Model license: MiniMax H3 Community License, https://platform.minimax.io/h3-license
...
# ======================================================================
# L40S native-runtime configuration. CUDA PyTorch is provided by the host.
# This edition does not replace Torch or install Blackwell-only wheels.
...
def _l40s_env_bool(name, default):
    return _os.environ.get(name, "1" if default else "0").strip().lower() not in {"0", "false", "no", "off"}

_l40s_boot_started = __import__("time").perf_counter()
L40S_MEMORY_OPT = _l40s_env_bool("H3_L40S_MEMORY_OPT", True)
L40S_CONDITION_CACHE = _l40s_env_bool("H3_L40S_CONDITION_CACHE", True)
L40S_COMPILE = _l40s_env_bool("H3_L40S_COMPILE", False)
L40S_CHUNK_ROWS = int(_os.environ.get("H3_L40S_CHUNK_ROWS", "4096"))
L40S_CACHE_BYTES = int(_os.environ.get("H3_L40S_CACHE_MIB", "256")) * 1024**2
L40S_RESERVE_GIB = float(_os.environ.get("H3_L40S_RESERVE_GIB", "6"))
if not 256 <= L40S_CHUNK_ROWS <= 65536:
    raise ValueError("H3_L40S_CHUNK_ROWS must be between 256 and 65536")
if not 0 <= L40S_CACHE_BYTES <= 4096 * 1024**2:
    raise ValueError("H3_L40S_CACHE_MIB must be between 0 and 4096")
if not 2 <= L40S_RESERVE_GIB <= 16:
    raise ValueError("H3_L40S_RESERVE_GIB must be between 2 and 16")
if _os.environ.get("H3_CU130_CHILD") == "1":
    raise RuntimeError("Start this L40S edition in a fresh native CUDA runtime, not the private SM120 child.")
if "comfy.model_management" in _sys.modules:
    raise RuntimeError("Restart the Python runtime before running this L40S edition; Comfy's GPU/attention policy is already initialized.")
_os.environ["H3_GPU_PROFILE"] = "l40s_48gb"
_os.environ.setdefault("PYTORCH_CUDA_ALLOC_CONF", "expandable_segments:True")
_cache_root = _pl.Path(_os.environ.get("H3_L40S_CACHE_DIR", "/content/.h3_l40s_cache"))
...
_pl.Path("/content").mkdir(parents=True, exist_ok=True)
...
# Execute directly in the installed CUDA environment. Do not restore a Blackwell
# runtime snapshot or install a different Torch into a running notebook.
_CU130_CHILD = True  # legacy main-body guard; no child process in this edition


# L40S native CUDA runtime continues below.
if _CU130_CHILD:
    ...
    # Library mode does not require terminating other ComfyUI or tunnel processes.
    ...
    # TF32 and quantized attention may change floating-point results slightly.
    # These settings do not change steps, scheduler or LoRA strengths.
    ...
    def _l40s_tensor_tree(value, *, clone=False):
        """Copy only known conditioning types; never cache opaque mutable state."""
        if torch.is_tensor(value):
            if value.layout != torch.strided:
                raise TypeError("non-strided tensor is not cacheable")
            tensor = value.detach().to("cpu")
            return tensor.clone() if clone else tensor
        if type(value) is dict:
            return {k: _l40s_tensor_tree(v, clone=clone) for k, v in value.items()}
        if type(value) is list:
            return [_l40s_tensor_tree(v, clone=clone) for v in value]
        if type(value) is tuple:
            return tuple(_l40s_tensor_tree(v, clone=clone) for v in value)
        if value is None or type(value) in (str, int, float, bool):
            return value
        raise TypeError(f"uncacheable conditioning value: {type(value).__name__}")


    def _l40s_tree_bytes(value):
        if torch.is_tensor(value):
            return value.numel() * value.element_size()
        if isinstance(value, dict):
            return sum(_l40s_tree_bytes(v) for v in value.values())
        if isinstance(value, (list, tuple)):
            return sum(_l40s_tree_bytes(v) for v in value)
        return len(value.encode("utf-8")) if isinstance(value, str) else 32


    def _l40s_token_digest(value):
        """Include image/video tensor CONTENT, not object id or just the prompt."""
        import hashlib
        ...
        def visit(item):
            ...
            if torch.is_tensor(item):
                if item.layout != torch.strided:
                    raise TypeError("unsupported token tensor layout")
                ...
                raise TypeError(f"unsupported token type: {type(item).__name__}")
            digest.update(b";")
        visit(value)
        return digest.hexdigest()


    def _l40s_cache_encoder(clip):
        """Cache this CLIP instance only. Fresh model loads get fresh caches."""
        from collections import OrderedDict
        if not L40S_CONDITION_CACHE or getattr(clip, "_l40s_cached", False):
            return
        original = getattr(clip, "encode_from_tokens_scheduled", None)
        if not callable(original):
            log("  L40S conditioning cache unavailable on this CLIP API")
            return
        entries = OrderedDict()
        total_bytes = 0
        def encode_cached(*args, **kwargs):
            nonlocal total_bytes
            # Hooked/scheduled encoder state is not represented by tokens alone.
            patcher = getattr(clip, "patcher", None)
            hooks = getattr(patcher, "forced_hooks", None)
            patched = bool(getattr(patcher, "patches", {}))
            key = None
            if hooks is None and not patched and getattr(clip, "apply_hooks_to_conds", None) is None:
                try:
                    key = _l40s_token_digest((args, kwargs, getattr(clip, "layer_idx", None)))
                except (TypeError, ValueError):
                    pass
            if key is not None and key in entries:
                saved, size = entries.pop(key)
                entries[key] = (saved, size)
                log("  L40S conditioning cache HIT: reusing exact prompt + media encoding")
                return _l40s_tensor_tree(saved, clone=True)
            # Evict the DiT before the 25 GiB encoder loads. Cache hits avoid this.
            torch.cuda.synchronize()
            mm.free_memory(1e30, mm.get_torch_device(), keep_loaded=[
                lm for lm in getattr(mm, "current_loaded_models", [])
                if _same_patcher(getattr(lm, "model", None), patcher)
            ])
            result = original(*args, **kwargs)
            if key is not None:
                try:
                    size = _l40s_tree_bytes(result)
                    if size <= L40S_CACHE_BYTES:
                        saved = _l40s_tensor_tree(result, clone=True)
                        while entries and (total_bytes + size > L40S_CACHE_BYTES or len(entries) >= 4):
                    ...
                    pass
            return result
        clip.encode_from_tokens_scheduled = encode_cached
        clip._l40s_cached = True


    def _l40s_memory_patch(model, lower_memory=False):
        """Use the upstream H3 execution policy, preserving checkpoint compatibility."""
        if not L40S_MEMORY_OPT:
            return model
        if "H3MemoryOptimization" not in N:
            raise RuntimeError("Update H3-Optimizations: H3MemoryOptimization is required for this L40S build.")
    ...
    def _l40s_compile_patch(model, params):
        if not L40S_COMPILE or str(params.get("_l40s_no_compile", "0")) == "1":
            return model, False
        if "TorchCompileModel" not in N:
            log("  L40S compile skipped: experimental Comfy TorchCompileModel node unavailable")
            return model, False
        # Comfy's wrapper owns cloning and offload integration. Never replace the
        # shared base module's forward method or attach CUDA graphs manually.
    ...
    def elapsed(fn):
        ...
        return a.elapsed_time(b) / 4
    rows = []
    for dtype in (torch.float16, torch.bfloat16):
        for qlen, klen, causal in ((1024, 1024, False), (1024, 1024, True), (1024, 1536, False), (4096, 4096, False)):
            q = torch.randn((1, 8, qlen, 128), device='cuda', dtype=dtype)
            k = torch.randn((1, 8, klen, 128), device='cuda', dtype=dtype)
            v = torch.randn_like(k)
            native = lambda: torch.nn.functional.scaled_dot_product_attention(q, k, v, is_causal=causal)
            sage = lambda: sageattn(q, k, v, tensor_layout='HND', is_causal=causal)
            ref, out = native().float(), sage().float()
            torch.cuda.synchronize()
            relative_rmse = ((ref - out).square().mean().sqrt() / ref.square().mean().sqrt().clamp_min(1e-8)).item()
            assert torch.isfinite(out).all() and relative_rmse <= 0.05, relative_rmse
            rows.append(dict(dtype=str(dtype), q=qlen, k=klen, causal=causal,
    ...
    if LOWVRAM_T4_PROFILE:
        ...
        if L40S_MEMORY_OPT:
            _required_nodes.append("H3MemoryOptimization")
        ...
        if CUDA_CONTEXT_POISONED:
            j.update(
                ...
                msg="The isolated H3 CUDA context previously reported an illegal memory access. "
                    "Restart the Python runtime and launch the L40S script before generating."
            )
            return
    ...
    def _generate(jid, p):
        ...
        try:
            ...
            try:
                gc.collect()
                # Keep allocator blocks reusable across L40S stages and warm jobs.
                if not L40S_PROFILE and not FULL_STACK_RESIDENCY:
                    mm.soft_empty_cache()
                    torch.cuda.empty_cache()
            except Exception:
                pass
            ...
            lower_memory = str(p.get("_force_te_evict", "0")) == "1"
            model = _l40s_memory_patch(model, lower_memory=lower_memory)
            j["l40s_memory_optimization"] = bool(L40S_MEMORY_OPT)
            j["l40s_lower_memory_retry"] = lower_memory
            ...
            try:
                ...
                _msg_low = _msg.lower()
                _fatal_cuda = any(x in _msg_low for x in (
                    "illegal memory access", "illegal instruction", "device-side assert", "misaligned address"))
                if compile_active and not _fatal_cuda:
                    retry_p = dict(p)
                    retry_p["_l40s_no_compile"] = "1"
                    raise GenerationRestart(retry_p, "l40s_compile_fallback", _msg) from _sparse_exc
                _convrot_float = (
            ...
            if (
                ...
                or "illegal instruction" in low_tb
                or "device-side assert" in low_tb
                or "misaligned address" in low_tb
            ):
                CUDA_CONTEXT_POISONED = True
                log(
                    "  ✗ CUDA context marked POISONED after illegal memory access. "
                    "Queued renders will not be started; restart the Python runtime before running this L40S script again."
                )
            ...
            extra = (
                "\nCUDA CONTEXT POISONED — restart the Python runtime before another render."
            ...
            if not CUDA_CONTEXT_POISONED:
                try:
                    ...
                    if any(x in msg for x in ("illegal memory access", "cudaerrorillegaladdress", "illegal instruction", "device-side assert", "misaligned address")):
                        CUDA_CONTEXT_POISONED = True
                        log(
                            "  ✗ CUDA allocator cleanup reported illegal memory access; "
                            "context marked POISONED. Restart the Python runtime."
                        )
                    else:
    ...
    def meta():
        ...
        return jsonify(
            ...
            l40s_profile=True, l40s_memory_opt=L40S_MEMORY_OPT,
            l40s_compile=L40S_COMPILE, l40s_condition_cache=L40S_CONDITION_CACHE,
            l40s_chunk_rows=L40S_CHUNK_ROWS,
            l40s_startup_sec=round(globals().get("L40S_READY_SECONDS", 0.0), 3),
...
{_h3_prompt_contract(mode, duration)}'''
    ...
    *{scrollbar-width:none}*::-webkit-scrollbar{display:none;width:0;height:0}
    ...
    if os.environ.get("H3_UI_BLOCKING_CHILD") == "1" or "google.colab" not in sys.modules:
        log("="*74)
        log(f"🚀 L40S H3 UI is READY on port {UI_PORT}")
        log("  Flask is running in the foreground. Put hosted access behind your authenticated proxy.")
        log("="*74)
